...
export class OllamaProvider implements AIProvider {
  ...
  async listModels(): Promise<Model[]> {
    try {
      ...
      throw new OllamaConnectionError(`Failed to list models: ${error}`);
    }
  }
  ...
  async* generateStream(prompt: string, options: GenerationOptions = {}): AsyncIterable<string> {
    try {
      ...
      throw new OllamaConnectionError(`Failed to generate response: ${error}`);
    }
  }
  ...
  async* chat(messages: ChatMessage[], options: ChatOptions = {}): AsyncIterable<ChatResponse> {
    try {
      ...
      throw new OllamaConnectionError(`Failed to chat: ${error}`);
    }
  }
  ...
  async getModelInfo(modelName: string): Promise<any> {
    try {
      ...
      throw new OllamaConnectionError(`Failed to get model info: ${error}`);
    }
  }
