...
class DocumentService {
    constructor(_configManager, logger) {
        this.supportedFormats = ['md', 'markdown', 'json', 'yaml', 'yml', 'txt'];
        this.logger = logger;
    }
    // 读取文档
    async readDocument(filePath, options = {}) {
        try {
            ...
            // 读取原始内容
            const rawContent = await fs_extra_1.default.readFile(filePath, encoding);
            // 解析内容
    ...
    // 写入文档
    async writeDocument(filePath, content, options = {}) {
        try {
            const { format = 'auto', encoding = 'utf8', createIfNotExists = true, backup = true } = options;
            const detectedFormat = format === 'auto' ? this.detectFormat(filePath) : format;
            // 检查文件是否存在
            ...
            // 写入文件
            await fs_extra_1.default.writeFile(filePath, serializedContent, encoding);
            // 获取新文件信息
            const stats = await fs_extra_1.default.stat(filePath);
            const metadata = {
                format: detectedFormat,
    ...
    async parseMarkdown(content) {
        ...
        if (frontmatterMatch && frontmatterMatch[1] && frontmatterMatch[2]) {
            try {
                result.frontmatter = yaml_1.default.parse(frontmatterMatch[1]);
    ...
    // 转换文档格式
    async convertDocument(sourcePath, targetPath, targetFormat) {
        try {
            // 读取源文档
            ...
            // 转换内容
            const convertedContent = await this.convertContent(sourceResult.content, sourceResult.metadata.format, targetFormat);
            // 写入目标文档
    ...
    // 私有方法：序列化内容
    async serializeContent(content, format) {
        switch (format) {
            case 'json':
            ...
            case 'markdown':
                if (typeof content === 'object' && content.frontmatter && content.content) {
                    const frontmatter = content.frontmatter ? `---\n${yaml_1.default.stringify(content.frontmatter)}---\n` : '';
    ...
    // 私有方法：分析文档结构
    analyzeStructure(content, format) {
        switch (format) {
            case 'markdown':
                return {
                    ...
                    maxHeadingLevel: content.headings?.reduce((max, h) => Math.max(max, h.level), 0) || 0
                };
            case 'json':
                return {
    ...
    // 私有方法：转换内容格式
    async convertContent(content, sourceFormat, targetFormat) {
